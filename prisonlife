...
do
	...
	if UIS.TouchEnabled and not UIS.MouseEnabled then
		Out.Size = UDim2.new(0.647, 0,0.684, 0)
		Out.Position = UDim2.new(0.176, 0,0.146, 0)
		CmdsIcon.Size = UDim2.new(0.092, 0,0.684, 0)
		Temp.Phone = true
	end
	]]
end
...
function AdminChatted(Text,Speaker)
	...
	Arg1_P = SPLITED[1]
	local CmdsList = "[ADMIN]: !bring player, !cmds, !criminal player, !goto player, !kill [player,all], !fling [player], !car, !loopkill [player], !unloopkill [player], !tase [player,all]"
	...
	if commandPLAYER("kill") then
		...
		if Arg2_P and Check1(Arg2_P) then
			Kill_All(Check1(Arg2_P))
			return
		end
		if r then
			MKILL(r)
